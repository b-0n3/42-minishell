...
/*   Updated: 2021/12/07 04:40:49 by am-khant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (this->cursor - this->l_cursor == 1 && (this->cmd[this->l_cursor] == '\"'
			|| this->cmd[this->l_cursor] == '\'' ))
		return (ft_strdup(" "));
...
t_token	*get_word(t_shell *this)
{
	...
	if (this->cursor <= this->l_cursor)
		return (NULL);
	token = new_token(ft_strndup(this->commmand + this->l_cursor,
